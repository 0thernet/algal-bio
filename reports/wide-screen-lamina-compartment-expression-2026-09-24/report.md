...
- **POSITIVE (libvar/ + libvar2/, below):** registered quantifications of GC-correlated same-condition library variation in public lamina DamID/pA-DamID tracks passed all pre-committed predictions in BOTH of two never-opened-cohort analyses (label SYSTEMATIC each): median |rho| 0.141 over 22 pairs in four cohorts and 0.159 over 46 pairs in a single larger cohort, scale-free/raw ratios ~0.97, and holdout/generality checks passed. Two-library GC-vs-lamina contrasts are not identifiable without within-cohort same-condition calibration or an explicit GC-conditioned model.
...
Combined reading across both studies: **68 same-condition pairs in two registered, never-opened-cohort analyses both pass every pre-committed prediction.** GC-correlated between-library gradients in public lamina DamID-family tracks are systematic, survive the strongest normalizations tested (z-scoring; depositor quantile normalization), reach canonical lamin-perturbation effect sizes — and in places exceed cross-condition and perturbation contrasts — while being absent in some conditions (K562 WT, OPC proliferating cells, ESC WAPL 0h/6h). Any GC-vs-lamina association from a two-library contrast is not identifiable without within-cohort same-condition calibration or an explicit GC-conditioned model.
