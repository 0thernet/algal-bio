...
## Status (2026-09-25): the GC-lamina claim is an unresolved direction; the campaign's positive result is the library-variation quantification
- **POSITIVE (libvar/ + libvar2/, below):** registered quantifications of GC-correlated same-condition library variation in public lamina DamID/pA-DamID tracks passed all pre-committed predictions in BOTH of two registered analyses, each including a never-opened holdout (label SYSTEMATIC each): median |rho| 0.141 over 22 pairs in four cohorts (one never opened) and 0.159 over 46 pairs in a single never-opened larger cohort, scale-free/raw ratios 0.96-0.97, and holdout/generality checks passed. Two-library GC-vs-lamina contrasts are not identifiable without within-cohort same-condition calibration or an explicit GC-conditioned model.
...
- The decisive test (mef/, GSE124205, mouse primary MEFs, shLaminA/C DamID) was registered and frozen before its data were downloaded, then fetched and run on 2026-09-25: **DIRECTION_ONLY in all three arms — not replicated**. Both knockdown primaries replicate the predicted direction (rho ~-0.35) but the between-embryo wild-type placebos reach 52-83% of the primary size and breach the registered bound, so the gradient cannot be attributed to lamin A/C loss rather than library-level variation. The libvar/ study then quantified that library-level variation itself — outcome sections below.
...
## Registered quantification of the GC library variation itself (libvar/): SYSTEMATIC — all four predictions passed, including the never-opened cohort

Why this study: after the mef/ outcome, the dominant fact in this record is that GC-correlated differences between same-condition libraries decided two registered labels. libvar/ quantifies that phenomenon directly, pre-registered (libvar/protocol.json) and frozen 2026-09-25T02:11:54Z before the holdout cohort was downloaded.

Design: 22 same-condition library pairs across 4 cohorts (K562 pA-DamID, RPE1 pA-DamID, MEF DamID, and a holdout: GSE135834 LMNB1 DamID in mouse primary oligodendrocyte lineage, depositor-quantile-normalized on mm10, never opened before freeze). Per pair: delta = A−B, baseline = mean(A,B), the same rank-partial-Spearman GC test as the earlier registrations (covariates tss_count + baseline, 5000 within-chromosome shifts), raw and scale-free arms. Registered predictions: P1 ≥60% of pairs |rho| ≥ 0.10; P2 median |scale-free/raw| ratio ≥ 0.5 (persistence beyond dynamic range); P3 median |rho| ≥ one third of the 0.249 median of this program's seven registered treatment contrasts; P4 ≥4/6 holdout pairs |rho| ≥ 0.05. Label SYSTEMATIC only if all four pass.

### Outcome (frozen 02:11:54Z; opc fetch 02:13Z; run 02:19Z; `libvar/intake.json`, `libvar/results/result.json`, `logs/libvar-run.log`)

**SYSTEMATIC.** Median same-condition |rho| = 0.141 (bound 0.083); 15/22 pairs (68%) have |rho| ≥ 0.10 (bound 60%); the scale-free/raw median ratio is 0.97, so the gradients are a rank-ordered structure, not a dynamic-range artifact; 5/6 holdout pairs pass (PF replicates near zero; the differentiated-oligodendrocyte T3 trio is +0.40/+0.23/−0.35 — larger than the nine cross-condition PF-vs-T3 pairs at +0.03 to +0.19). Cohort medians: K562 0.025, MEF 0.152, OPC 0.180, RPE1 0.296.

Reading: GC-correlated between-library gradients in public lamina DamID-family tracks are systematic, mostly survive the strongest normalizations in the pool (z-scoring; the holdout arrives already quantile-normalized), reach at least a third of canonical lamin-perturbation effect sizes — and in places exceed the cross-condition differences in the same cohort. A GC-vs-lamina association from a two-library contrast is not identifiable without within-cohort same-condition calibration or an explicit GC-conditioned model.

Required disclosures (post-outcome review `design/libvar-post-outcome-review.json`, PASS_WITH_REPAIRS; integrator re-verified all 25 frozen files, the freeze link, and all 32 data files):
- Pairs are not independent — one anomalous library supplies several pairs (the opc trios, the K562 WT trio, WT_x17 twice); '68% of pairs' is a pair-level measurement.
- Same-condition pairs across embryos/batches/experiments also carry biological replicate variation, so the measurement is conservative (larger) for a pure library-effect claim.
- K562 is the counterpoint inside the pool: all three WT pairs are near zero (cohort median 0.025), while the LMNA-KO clone-14 replicates r7-vs-r8 differ by −0.233 — a clone-specific batch effect, not a universal one.
- The 7 previously-computed pairs used a different baseline convention (independent third track vs mean(A,B)); they do not reproduce exactly — most visibly the mef x17 pairs (+0.045/+0.152 raw here vs +0.230/+0.234 under the earlier orientation/baseline; scale-free recovers +0.127/+0.191), explained by WT_x17's ~2-4x dynamic-range ratio.
- The cross-condition XC class is confounded: PF tracks normalize to Dam-2_4 and T3 tracks to Dam-12_13_14, so the all-positive XC sign partly reflects the different Dam reference pools.
- The GSE300197 (hiPSC) cohort was removed at pre-freeze review: its 'DNK vs mCh' pair is a dominant-negative-KASH adenovirus perturbation contrast, and the tracks are LMNB1 CUT&RUN, not DamID — it contributes no same-condition pair. Denominators amended 24 → 22 before freezing.
- DamID-seq and pA-DamID are deliberately pooled as one 'public lamina DamID-family' class; gc comes from gc5Base (hg38) or 2bit-derived (mm9/mm10).
- Prior art (design/prior-art-libvar.md): PARTIALLY_KNOWN. GC bias in sequencing counts and DamID normalization practice are documented (CQN; Daim; z-scoring in van Schaik 2022; GC-associated replicate variability in ChIP-seq). What appears new: a DamID-specific quantification of same-condition GC gradients and their unidentifiability consequence for GC-lamina claims.

Independent reviews: pre-freeze `design/libvar-review.json` (FAIL on first pass — the hipsc misclassification — repaired and re-frozen before any holdout download); post-outcome `design/libvar-post-outcome-review.json` (PASS_WITH_REPAIRS; integrator byte-level verification passed).

## Second holdout (libvar2/, GSE181693 mouse ESC LMNB1 pA-DamID): SYSTEMATIC again — the pattern generalizes

libvar2/ asks whether the libvar/ pattern is a quirk of those four cohorts or generalizes. GSE181693 (van Schaik et al. 2022; E14TG2a ESCs, AID degrons for CTCF, WAPL, both, RAD21; 0h-96h IAA time-course; LMNB1 pA-DamID, mm10) contributes **46 same-condition pairs in one cohort** — more than libvar/'s entire pool — registered, reviewed (`design/libvar2-review.json`, PASS_WITH_REPAIRS; one repair closed a single-genotype loophole in the generality check), frozen 2026-09-25T02:47:18Z, fetched 02:48Z, run 03:00Z.

**SYSTEMATIC.** Median |rho| = 0.159 (bound 0.083); 30/46 pairs (65%) ≥ 0.10; scale-free/raw median ratio 0.96; 4/6 condition groups pass the generality check spanning both degron genotypes (CW_0h 0.269, CW_6h 0.192, WAPL_24h 0.185, WAPL_96h 0.250).

The internal structure is informative: WAPL 0h and 6h same-condition trios are near zero (0.027, 0.009) — the pattern is per-condition library variation, not a uniform artifact — while the reported-only contrasts show the antibody-lot change pair at −0.238 and the WAPL timepoint perturbation contrasts at −0.114/−0.160, i.e. same-condition library variation here reaches and exceeds real perturbation contrasts. Drug-condition pairs are largest (class medians: drug 0.225 > IAA-treated 0.160 > untreated 0.105), consistent with nominal-treatment pairs absorbing real heterogeneity.

Required disclosures (post-outcome review `design/libvar2-post-outcome-review.json`, PASS; integrator re-verified all 14 frozen files, the freeze link, and all 58 data files):
- Single cohort: a cohort-specific artifact is not excluded; this tests generalization of libvar/, not independent rediscovery.
- Strong non-independence: CW_0h's six pairs reuse four tracks.
- Treated/drug-timepoint 'same condition' means same nominal treatment; the class-median ordering above is the expected signature of absorbed heterogeneity.
- The DMSO/EED/GSK *_24h condition labels rest on GEO titles/characteristics; the deposited filenames say '*_IAA_0h'.
- The depositor's z-score tsvs exist but were not used; the scale_free arm is our own per-track z-score.
- Prior art as in libvar/ (PARTIALLY_KNOWN).

Combined reading across both studies: **68 same-condition pairs in two registered analyses, each with a never-opened holdout, both pass every pre-committed prediction.** Two cosmetic code issues in frozen libvar2 files are recorded rather than edited, because editing would break the freeze (`design/pr11-review.json`). GC-correlated between-library gradients in public lamina DamID-family tracks are systematic, survive the strongest normalizations tested (z-scoring; depositor quantile normalization), reach canonical lamin-perturbation effect sizes — and in places exceed cross-condition and perturbation contrasts — while being absent in some conditions (K562 WT, OPC proliferating cells, ESC WAPL 0h/6h). Any GC-vs-lamina association from a two-library contrast is not identifiable without within-cohort same-condition calibration or an explicit GC-conditioned model.
