...
class TileMeans(unittest.TestCase):
    ...
    def test_bedgraph_reader_normalises_ensembl_chromosome_names(self):
        with tempfile.TemporaryDirectory() as d:
            p = Path(d) / "x.bedGraph.gz"
            with gzip.open(p, "wt") as fh:
                fh.write('track name="R225R_15_1_HiC_500KB_Active PC1" type=bedGraph\n')
                fh.write("1\t500000\t1000000\t0.25\n")
                fh.write("X\t0\t500000\t-0.75\n")
                fh.write("chr2\t0\t500000\t1.5\n")
            per = C.read_bedgraph(p)
            self.assertEqual(sorted(per), ["chr1", "chr2", "chrX"])
            tiles = [("chr1", 0, 500000), ("chr1", 500000, 1000000), ("chr2", 0, 500000)]
            m, _ = C.bedgraph_tile_means(p, tiles, 0.5)
            self.assertTrue(np.isnan(m[0]))
            self.assertAlmostEqual(m[1], 0.25)
            self.assertAlmostEqual(m[2], 1.5)
