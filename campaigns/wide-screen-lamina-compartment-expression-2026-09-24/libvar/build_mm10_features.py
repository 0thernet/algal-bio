#!/usr/bin/env python3
"""Mouse (mm10) 500 kb tile features for the libvar/ campaign, built from genome annotation only.

Inputs in libvar/ref (UCSC goldenPath/mm10; hashes written to the manifest): mm10.2bit, refGene.txt.gz,
...
Columns mirror the hg38 features used by damid/:
  gc        fraction G+C over non-N bases of the tile (the hg38 gc came from the UCSC gc5Base track,
            which mm10 lacks); NaN when n_frac > 0.5 (hg38: gap_frac > 0.5)
  ...
  tss_count TSS count per tile, one TSS per gene symbol: the 5' end base of that symbol's longest refGene
            transcript (txStart on +, txEnd - 1 on -) (hg38 used RefSeq Select, one transcript per gene; mm10 has no Select track)
  cpg_n     CpG islands whose start lies in the tile (reported-only robustness covariate)
Writes libvar/features/mm10_tiles.tsv and libvar/features/manifest.json; prints counts only."""
...
def main():
    ...
    ins = {f: sha(REF / f) for f in ("mm10.2bit", "refGene.txt.gz", "cpgIslandExt.txt.gz")}
    tb = twobit_tiles(REF / "mm10.2bit", CHROMS, TILE)
    ...
    with open(OUT / "mm10_tiles.tsv", "w") as fh:
    ...
    json.dump({"schema": "bio.libvar-features.v1", "assembly": "mm10", "tile": TILE, "chromosomes": CHROMS, "n_tiles": n,
               "chrom_sizes": {c: tb[c][0] for c in CHROMS}, "inputs_sha256": ins,
               "input_sources": {f: "https://hgdownload.soe.ucsc.edu/goldenPath/mm10/" + ("bigZips/" if f.endswith("2bit") else "database/") + f for f in ins},
